...
|---|---|---|---|
| **G-1** | **DD-1 "Lint passes."** | CI's lint step runs with `continue-on-error: true` and an in-file comment saying not to flip it "until the backlog is paid down" (`architecture-assessment/02-Architecture-Strengths.md` §186). **No task among the 63 flips it.** The obligation therefore has no *mechanical* enforcement outside the local husky hook — and at team size 1 there is no second party to notice a bypass. `01-Architecture-Principles.md` §52 and §284 cite the non-blocking lint *approvingly*, as precedent for ratchet enforcement, so the corpus never treats it as a gap. | **PROH-20** (honor-system). The program flips boundary rules to `error` from Sprint 4 onward and relies on a gate CI does not enforce. Recommend recording this as a known standing condition. |
