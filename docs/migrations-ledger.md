...
| ID       | Change                                               | Intended end state                                                                                                                                                    | Current stage                                                                                                                                                                                                                             | Owner         | Review-by | Gate                                                                              |
| -------- | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | --------- | --------------------------------------------------------------------------------- |
| `LDG-17` | Route cross-feature imports through barrels (T-101b) | No cross-feature import reaches a path deeper than `@/features/<f>` or `@/features/<f>/server`; the boundary lint rule is `error` with only the enumerated exceptions | **Cycle removed; migration ready to retry.** Barrels published (T-101a, `cbf210a`); the migration was attempted and reverted, then its blocker was cleared — T-102a added notifications' act-side seam and T-102b moved the back-edge onto it, so `features/notifications` now imports no feature at all | NgnPhamGiaHuy | Sprint 6  | ~~T-102a/b~~ **cleared**; retry uses ADR-101 Amendment 1's two entry points |
