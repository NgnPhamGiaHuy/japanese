...
| **PF-9** | **Agree the branch and rollback convention:** each sprint ends on a tagged deployable commit; sprint PRs are the revert unit | ~~There is no CI/CD pipeline and no deployment history (R-13)~~ — **false; corrected 2026-08-04:** `.github/workflows/ci.yml` predates this corpus (`15e203b`, 2026-07-16), runs 5 jobs, and lint is blocking. "Rollback" still means revert-to-previous-sprint, and that still works only if boundaries are tagged. Note `10-Release-Plan.md:221` later overrode the revert unit to **the release unit, not the PR** | All sprints; X-2, X-6 |
