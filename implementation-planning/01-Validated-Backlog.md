...
**Delivered** `LDG-02` (the type-vocabulary row) is **closed**: T-108a reached its stated end state exactly — `NotificationType` now equals `NotificationKind | "digest"`, verified not assumed, via `check-vocabulary-agreement.mjs`'s `NotificationType` target flipped from `report-only` to `enforce` and passing (`agrees (10 values)`). `LDG-01` (the legacy-shape row) **stays open**, updated with the precise, current-as-of-this-wave facts from T-108b/c/d's own re-verification: no `.firebaserc` exists (so NQ-1 isn't just unanswered, it's unanswerable at the repo level — a stronger, more precise finding than the row previously stated), the backfill script and 4 legacy fields are all still in place, and the dual read-path/index machinery T-108d would collapse is confirmed still fully wired. `domain/events.ts`'s in-code retirement comment (previously "the two are reconciled as producers migrate," naming no end state) now points at the ledger rows by ID rather than restating their content inline. Every stage claim above is cited against a specific file this task or T-108a/b/c/d directly read this wave, not assumed from the prior audit.
