...
**Size** M · **Wave** 6 · **Status** ✅ **DONE**
**Traces to** ADR-105 (AD-05) · W-5, RC-8, CX-9, TD-10, OP-17, PC-15 · cluster **C4** · `assess/03`, `/06`, `/05`, `/09`
**Delivered** `features/kana/survival/` now exists and mirrors `features/kana/quiz/`'s exact shape: `components/` (the 4 screens, moved verbatim — confirmed via research they had exactly one importer, the route's own `page.tsx`, so zero fan-out import updates were needed) plus a new `KanaSurvival.tsx` root orchestrator; `hooks/` (`useSurvivalGame.ts`, `useDropMode.ts`, both moved from the shared root `features/kana/hooks/` since both were survival-exclusive, confirmed by grep — `useKanaQuizSession`/`useKanaDataset` stayed put since quiz uses them too); a new `types.ts` (`SurvivalPhase`, `ChallengeMode`, `DropWord`, moved out of the shared `kana.types.ts` since grep confirmed zero consumers outside survival); and an `index.ts` barrel exporting `KanaSurvival` + the two Time-Attack constants + `useSurvivalGame`, matching quiz's barrel shape. `app/[locale]/(immersive)/kana/survival/page.tsx` shrank from 71 lines of hook-wiring/phase-branching to a 4-line thin orchestrator (`return <KanaSurvival />`), identical in shape to quiz's own 9-line page. The `app → game` edge is gone: research confirmed the 4 screens' `@/features/game` imports were the only `app/**` → `features/game` edges anywhere in the repo; after the move, zero remain. Within-module imports follow the established sibling convention exactly (verified against `QuizPlaying.tsx`'s own precedent): survival-local references use relative paths (`../hooks`, `../types`), while references to genuinely shared root-level kana infrastructure use direct absolute paths to that subdirectory (`@/features/kana/hooks`, `@/features/kana/types`, `@/features/kana/components`, `@/features/kana/actions`) rather than round-tripping through the top feature barrel. `features/kana/index.ts` updated: `KanaSurvival` added alongside its 4 siblings, the Time-Attack constants/`useSurvivalGame` re-pointed from `./hooks` to `./survival`, and the stale "screens still live in the route layer" docblock note replaced with a completed-state description. Verified via a full production build (`npm run build`) — clean typecheck, all kana routes (including `/kana/survival`) statically generated with no errors — and `npm run lint` (0 errors, same pre-existing warning baseline). Full interactive gameplay verification in the browser was not possible: this app gates every in-app route behind Google OAuth (the same limitation already established for the admin dashboard and notifications page earlier in this engagement), and an unauthenticated visit to `/kana/survival` client-redirects to the login screen rather than rendering the game — confirmed this redirect is clean (no server/console errors) but could not click through an actual survival session. Confidence instead rests on: the successful build/typecheck, a line-by-line diff showing the 4 screens moved with zero logic changes (only import paths), and `KanaSurvival.tsx` being a verbatim structural copy of the original `page.tsx`'s hook-wiring and phase-branching logic. Incidental fix noticed and corrected while investigating the preview setup: the repo-root `.claude/launch.json` (outside `src/`, so outside T-119e's original Storybook-removal search scope) still had a `japanese-app-storybook` launch entry pointing at the `storybook` npm script T-119e deleted in Wave 5 — removed that dead entry.
...
**Delivered** Audited every `_components/` directory under `app/` (4 total, 11 files) by tracing each barrel import to its real defining module rather than trusting the import path alone — a barrel re-export can still be a domain/hooks reach even when the importing line only names the top-level feature barrel. Found exactly one still-open violator beyond kana-survival's already-tracked 4 screens (T-105a): `NotificationsVirtualList.tsx`, importing `flattenNotificationGroups` from `features/notifications/domain/format.ts`. Relocated it (plus its co-located `.browser.test.tsx`) into `features/notifications/components/`, added both to that directory's barrel and the feature's root barrel (`NotificationsVirtualList` alongside the existing `NotificationRow`), and repointed `notifications/page.tsx`'s import from the deleted route-local path to the feature barrel. Verified via a real-browser test run (5/5 pass at the new location), the full unit suite (361/361 pass), a clean production build, and clean lint. The remaining 5 files (`ErrorFallback.tsx`, `MaintenanceScreen.tsx`, `ReactScan.tsx`, `BottomNav.tsx`, `NotificationsPlaceholders.tsx`) confirmed as genuine route chrome — none imports a feature's `hooks/`, `domain/`, or `services/`; `BottomNav.tsx`'s `useAdminRole`/`useNotifications` resolve to each feature's `context/` (its own documented composition-root public surface), not `hooks/`, so it passes the letter of the rule cleanly. Wrote the placement rule itself into `architecture-decision/04-Coding-Standards.md`'s CS-7 as a new, explicit, mechanical tiebreaker (grep-verifiable at review time: does the file import anything from a feature's `hooks/`, `domain/`, or `services/`, directly or transitively through a barrel — if so it's misplaced feature code regardless of tenure under `app/`), closing the gap the backlog task named — CX-9's supporting evidence was already cited throughout CS-4/CS-7, but no prior text stated an operational test a reviewer could apply file-by-file.
...
**Size** L · **Wave** 6 · **Status** ✅ **DONE**
**Traces to** ADR-104 (AD-04) · W-4, CX-2, OP-18, R-4 · cluster **C15** · `assess/03`, `/05`, `/09`, `/11`; `disc/11`
**Delivered** The flat 27-file `components/` directory no longer exists (confirmed: the directory itself is gone, not just emptied). Its files, plus the single-cluster hooks that were scattered across the root `hooks/` directory, sorted into 4 clusters by same-feature import-graph analysis (every file's within-flashcard imports were traced before deciding a home, not guessed from filenames) and landed as follows:
- **Study/SRS** (8 components + 2 hooks, 1,398 lines) → `games/study/components/` and `games/study/hooks/`, finishing a seam that was already partially built (the mode-selector shell existed there; the 3 practice screens and their shared primitives did not). `games/study/` is a named sub-module with its own barrel exactly as the acceptance criteria requires — it is **not** promoted to a flashcard-root sibling; see the "contested seam / naming" note below for why.
- **Sharing + Comments** (9 components + 3 hooks) → new `sharing/` sub-module: share files flat in `sharing/components/`, comment files in `sharing/components/comments/` (a real but non-interdependent sub-cluster — confirmed zero imports between the two clusters before merging them under one ADR-named sub-domain), all 3 hooks in `sharing/hooks/`.
- **Import/AI** (10 components + 2 hooks) → new `builder/` sub-module (see naming note below).
- 3 single-file hooks moved to the existing sub-module their sole consumer already lived in: `useVisibility` → `dashboard/hooks/` (consumer: `DeckCard.tsx`), `useCommentCount` → `detail/hooks/` (consumer: `CardCommentBadge.tsx` — a detail-view display concern, kept apart from the `sharing/` comments *engine* it displays a count for), `useMatchGameStore` → `games/match/hooks/` (4 consumers, all within `games/match/`). `useFlashcardGameBestScore` (used by **both** Match and Speed) moved to the pre-existing shared `games/hooks/`, joining `useGameCompletionLogger`.

**Naming decision, made explicit (not left implicit, per CS-8):** the sub-module the ADR calls "import/AI" is named `builder/`, after its actual root orchestrator (`LessonBuilder.tsx`/`useLessonBuilder`) — the ADR's own phrase describes two of the builder's internal tabs (paste/upload import, AI generation), not the whole sub-module: `DraggableCard` (card editing) and `LessonBuilderMeta` (title/description/theme) are neither. Documented in `builder/index.ts`'s own header so a reader searching for "where did import/AI go" finds the reasoning at the point of divergence.

**Contested-seam decision, made explicit (the acceptance criteria's own requirement):** `progress.service.ts` and `domain/` (SRS math, `CardWithProgress`) stay at the flashcard root rather than moving into `games/study/` — tracing the real call graph found **4** consumers, not 2: `games/study/` (heaviest — grades every answer), `games/match/` (`useMatchScoring` grades the matched card as "Good" on every pair), `services/shared.service.ts` (merges progress into the shared-deck read path), and externally `features/home` (read-only, via the root-barrel-exported `useDeckProgressStatus`). Moving either into `study/` would have made 3 of those 4 consumers reach across a new internal boundary for something genuinely shared, not study-owned. Documented directly in `features/flashcard/index.ts`'s own top docblock — the natural place a future contributor asking "why does this live here" would look — rather than in a separate design note nobody would find.

**Study/SRS-stays-nested decision, made explicit:** the ADR's own text lists "study/SRS" and "games" as separate items in its six-sub-domain enumeration, which could be read as calling for `study/` to be pulled out as a flashcard-root sibling of `games/`. Decided against: OP-18 (the evidence ADR-104 itself cites) already treats `games/` as **one** pre-existing partial seam, not two; match/speed/study already share `games/hooks/` and an identical intro-or-setup → playing → results phase-router shape; and re-nesting would be pure import-path churn on `games/study/index.ts` (already a real, working barrel exporting `StudySession`, already correctly consumed by the root barrel) for no functional or clarity gain. Documented in `features/flashcard/index.ts`'s docblock alongside the progress decision.

Root barrel (`features/flashcard/index.ts`) stays a curated list, not `export *` — only 2 of its ~28 export lines changed source path (`LessonBuilder`: `./components/LessonBuilder` → `./builder`; `ShareModal`: `./components/ShareModal` → `./sharing`); every exported symbol name is byte-identical to before, confirmed both by direct diff and by the clean production build (which compiles every external consumer). Verified **zero** cross-sub-module deep imports bypass a barrel (repo-wide search for `@/features/flashcard/<sub-module>/components|hooks` from outside that sub-module: zero hits) — the two real cross-sub-module edges this move created (`dashboard/` → `sharing/` for `ShareModal`, `detail/` → `sharing/` for `CommentPanel`) both go through `sharing/index.ts`.

**CS-2 file-size ceiling — a pre-existing violation, flagged not fixed:** `games/study/components/FlashcardPractice.tsx` (408 lines) is over CS-2's Hard (>400) tier — it was already at 396 when CS-2 was written and has since grown to 408, independent of this move (moving it changed only its own import lines, not its body). Splitting it is real, separate refactoring work with its own risk profile — out of scope for a relocation task, and CS-2's Hard tier is not yet lint-enforced (`04-Coding-Standards.md`'s own "Conflicts" section: turning it to `error` is gated on a prerequisite split elsewhere). Not silently ignored: flagged here explicitly, matching how the pre-move research surfaced it.

Verified with the full suite: production build (clean typecheck, every route including all `/flashcard/*` segments compiles), `npm run lint` (0 errors, same 102-warning baseline), unit tests (361/361), browser tests (76/76 across all 18 files, including the 2 relocated flashcard suites), emulator tests (129/129, 3 skipped — unaffected, confirming `services/`'s untouched location), and `npm run check:vocab` (all 3 vocabularies still agree). Two real import bugs caught and fixed via the build's own typecheck (not by inspection): a dynamic `import("../utils/parser")` in `LessonBuilderImportPane.tsx` that a static-import-only grep sweep had missed, and a stale `MatchItem` type import in `games/match/hooks/matchGrid.ts` (untouched by the move itself, but referencing `useMatchGameStore` transitively through the old root hooks barrel) — both are the kind of finding a full build catches and a diff-review alone would not.
...
**Delivered** Added `import/no-restricted-paths` zones for flashcard's 7 internal sub-modules (`dashboard`, `detail`, `games/match`, `games/speed`, `games/study`, `sharing`, `builder`) to `eslint.config.mjs`, `error` severity, each protecting its own internals unless reached via its own `index.ts`. Layered as additional zone entries on the **same** `features/*/**/*.{ts,tsx}` rule invocation T-101c already owns, rather than a second `import/no-restricted-paths` config block — ESLint flat config does not merge a rule's settings across config objects for the same files, so a separate block would have silently replaced T-101c's cross-feature zones instead of adding to them. Extended the protection one level further than the acceptance criteria's literal "cross-sub-module" wording: flashcard-root shared infrastructure (`hooks/`, `services/`, `domain/`, `types/`, `utils/`, `actions/`, `context/`, `loaders/`) also cannot deep-reach into a sub-module's internals, only its barrel — closing the same generator of drift one level out, since a deep import bypasses a sub-module's barrier exactly as much whether it originates from a sibling sub-module or from shared root code.

Landing the rule surfaced 3 real, pre-existing violations (not introduced by T-104a's move) that a clean-lint-run requirement doesn't let stand: `matchGameMode`/`speedGameMode` were reached via `games/match/config` / `games/speed/config` directly from `features/flashcard/index.ts` (the root barrel itself), `dashboard/hooks/useDashboardState.ts`, and both `loaders/flashcard-loader.ts` and `loaders/useFlashcardLoader.ts` — 4 call sites across 4 files. Fixed at the source rather than exempting: added `matchGameMode` to `games/match/index.ts`'s barrel and `speedGameMode` to `games/speed/index.ts`'s barrel (both were already meant to be public — every one of their call sites is outside the owning sub-module), then repointed all 4 call sites from the deep `.../config` path to the sub-module barrel.

Verified the rule actually fires, not just passes silently: injected a temporary probe file (`dashboard/components/_lint_probe.tsx`) importing `ShareModal` via the deep path `@/features/flashcard/sharing/components/ShareModal` — confirmed `eslint` failed it with the expected ADR-104-naming message — then confirmed the same probe importing `{ ShareModal }` from `@/features/flashcard/sharing` (the barrel) passed cleanly, and deleted the probe. Confirmed composition with T-101c/T-103b: full lint run is 0 errors (same 102-warning baseline as before this task), full production build clean, unit tests 361/361, browser tests 76/76.
...
**Delivered** `DeckDetailsPanel.tsx` and `AdminSidebar.tsx`'s mobile-drawer backdrop now import `DIALOG_BACKDROP_CLASSNAME` from `DialogChrome.tsx` instead of hardcoding their own className (AdminSidebar's was already byte-identical plus `lg:hidden`; DeckDetailsPanel's `bg-[#3c3c3c]/30` converges to the standard `bg-black/40`, an intentional visual unification, not a bug — the two prior variants were drift, not a deliberate design difference). `CommandPalette.tsx` (already a byte-identical literal duplicate) now imports the same constant instead of hand-copying it — removes an accidental-sync hazard, no visual change. `ShareModal.tsx` had no separate backdrop node at all (its `Dialog.Popup` fused backdrop+centering+panel into one className) — split it into a proper sibling `Dialog.Backdrop` (the shared constant) plus a `Dialog.Popup` retaining only its centering/layout classes, matching `Modal.tsx`'s established structure; visually identical (`bg-black/40 backdrop-blur-sm` was already the fused value) and behaviorally identical (`disablePointerDismissal` already blocks click-outside, so backdrop/popup click-target layering carries no functional risk). Per this task's own acceptance text ("dismiss behavior, focus handling and layout are unchanged — only backdrop styling unifies"), close-button styling was deliberately left untouched everywhere, including `DeckDetailsPanel`'s and `AdminSidebar`'s own custom close buttons — read "routes...close affordance through DialogChrome" in the same acceptance criteria as describing the toolkit `DialogChrome` offers Tier-2 composers, not a mandate that every bespoke close button must become the literal `DialogCloseButton` (which has no `aria-label` prop today, and `DeckDetailsPanel`'s existing custom button carries one — silently dropping it to force a component swap would have been a real a11y regression for a change this task explicitly scoped as backdrop-only). Documented the two-tier pattern (Tier 1 pre-built `Modal`/`ConfirmModal`, Tier 2 bespoke composers `DeckDetailsPanel`/`AdminSidebar`/`ShareModal`/`CommandPalette`) directly in `DialogChrome.tsx`'s own doc comment — the single natural, discoverable home for it — rather than scattering the same explanation across every consumer. Raw-hex cleanup on touched surfaces: `DeckDetailsPanel`'s hex is eliminated by the backdrop convergence itself; `ShareModal`'s "copied" state (`border-[#58cc02] bg-[#f2fbf0]`) converges onto the pre-existing `SEMANTIC_STATUS.success` token family (`border-hiragana/20 bg-hiragana/10`, `shared/utils/colors.ts`) — `#58cc02` was an exact `hiragana` match and `success` is precisely the semantic category a "link copied" confirmation represents, so this is a real, well-supported adjudication, not a nearest-match guess; `AdminSidebar`/`CommandPalette` had zero raw-hex to begin with. `project-discovery/04-Component-Inventory.md`'s `DialogChrome` entry and cross-cutting "Dialog stack" note updated to match the new consumer list.
...
**Delivered** `LogsVirtualList.tsx` now sources its rows through `useDataTable` (the same engine Users/Content run on) instead of iterating the raw `logs` array directly — `table.getRowModel().rows` feeds the virtualizer, with `rows[i].original` handed to `LogRow` exactly as `logs[i]` was before. Mirrors `DecksTable.tsx`'s own precedent exactly: same external prop interface (`logs: AdminLog[]`), same rendered output, `enableSorting`/`enableFiltering` both off since neither was ever wired to anything here (Reports' filtering is a server round-trip via `AdminLogFilters` → `applyLogFilters`, untouched by this migration — confirmed by not touching `log.service.ts`/`admin.actions.ts` at all) and none is added. Only one column is defined (`LOG_ROW_COLUMN`, a placeholder — `LogRow` renders one log as a single unified card, there's no per-column cell layout to define the way Users/Content have), documented as such so a future reader doesn't mistake it for an oversight. `useDataTable` itself gained one small, directly-motivated extension: an optional `getRowId` passthrough (`UseDataTableProps.getRowId`) — without it, the engine's default index-based `row.id` would have handed the virtualizer's height-measurement cache the wrong key on every page/filter transition (a taller/shorter log landing on a previously-measured index would flash the previous log's cached height until the next scroll-triggered remeasure), a real virtualization regression the task explicitly warned against. `getRowId: (log) => log.id` restores the exact same real-Firestore-ID keying the code already had. Users/Content are unaffected (neither passes `getRowId`, so both keep their existing index-based `row.id` unchanged). Raw-hex cleanup on touched surfaces (UR-3): `AdminPageHeader.tsx` and `AdminDateRangeFilter.tsx`'s `shadow-[#1cb0f6]` → `shadow-katakana` (exact match — both already sit directly beside `bg-katakana` in the same className, confirming it was the same color, just not yet using the token name). `logMeta.ts`'s 4 remaining hex values (`#7c3aed`, `#b86800`, `#3d8f00`, `#ffc8c9`) were investigated and adjudicated explicitly rather than converged: each is a deliberately darker/higher-contrast variant of its row's own already-tokenized color (chosen for legible badge text or a visible hover step on a light tint), not an un-extracted copy of an existing exact value — converging them onto the lighter token names would visibly change the rendered color, precisely the silent-nearest-match failure the `#ffc800` history warns against. Documented this reasoning directly in `logMeta.ts`'s own file header rather than silently leaving the hex in place unexplained. Also corrected `applyLogFilters`'s stale docstring (previously claimed "used by both the server-side Service and client-side Hook," which research confirmed was never true) to describe its actual, current single server-side caller. Verified via a full production build, `npm run lint` (0 errors, same pre-existing warning baseline), the full unit suite (361/361), and both `LogsVirtualList.browser.test.tsx` (all 4 tests, including the 500-row windowing check and the expand-to-remeasure check) and `DataTable.browser.test.tsx` (unaffected by the new optional `getRowId` param) passing unmodified at their existing assertions. `useDataTable` stays exactly where it was (`features/admin/hooks/`, admin-internal only) — no third consumer exists yet, so no lift to `shared/` per the three-use rule.
