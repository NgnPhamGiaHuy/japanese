...
- **The `_components/` placement tiebreaker (T-105b, closes CX-9's absent-tiebreaker gap).** A file under `app/**/_components/` is legitimate route chrome only if it imports **nothing** from any feature's `hooks/`, `domain/`, or `services/` (directly, or transitively through a barrel re-export that resolves to one of those — a type-only import, or a value that resolves to a feature's `components/`/`context/`, does not itself trigger this). If it imports any of those, it is feature code that happens to sit in the wrong layer — relocate it into the owning feature (its own `components/`, or a per-feature area) regardless of how long it has lived under `app/`. This is a mechanical test, verifiable by grep at review time; it is what fixes the generator of the CX-9 gap, not just a one-time sweep of today's instances.
...
6. **Pagination — pick per channel, no third mechanism.** One-shot administrative lists use cursor-token pagination (`useCursorPagination` — Firestore cursors are sequential, so this accumulates a `pageTokens[pageIndex] -> cursor` map rather than computing an offset). Realtime feeds use grow-window resubscribe (`subscribeNotifications`'s pattern — the live window's `limit` grows instead of layering a separate one-shot "older page" cache, trading read-amplification for one always-live source of truth). **No third pagination mechanism may be added** — no offset pagination, no `useInfiniteQuery` (both confirmed absent repo-wide). A new paged surface picks whichever of the two its data channel dictates (ADR-112).

**Why.** R-1 (per-mount listener multiplication: `useUserProgress` opens one listener per component across 10 mount sites, vs the single centralized notifications listener — the model to copy), R-2 (unbounded `subscribePublicLessons` streams the entire public corpus, live, to every dashboard visitor; no `limit()`, no virtualization), W-18 (per-keystroke `fetchLogsAction`; grow-window resubscribe re-reads the whole window), AD-13/AD-14. S-18 documents the LazyMotion/virtualization mechanisms as strengths to preserve; PC-4 documents the two virtualizer variants and their in-code rationale. AD-12/ADR-112 codifies the two pagination mechanisms as THE two: the divergence is channel-forced (one-shot Admin-SDK fetch vs `onSnapshot` stream), not accidental plurality, and the grow-window's read-amplification is a recorded deliberate correctness tradeoff, not unmanaged debt to "fix" (OP-3 rates unification headroom Low for exactly this reason).

**Example.** Good: `NotificationsContext` single app-lifetime listener; the two virtualized lists with in-code rationale (S-18, PC-4); `useCursorPagination`/`subscribeNotifications` as the two sanctioned pagination mechanisms, each documenting its own channel rationale in-code (AD-12). Bad: `subscribePublicLessons` (R-2, no `limit()`); admin log search firing a query per keystroke (W-18); introducing offset pagination or `useInfiniteQuery` as a third mechanism.
...
- **CS-10 Performance:** bounded queries, centralized per-entity listeners, debounced server inputs, virtualize-by-trigger, pagination picked per channel with no third mechanism, LazyMotion `m.*` only (R-1, R-2, W-18, AD-12/13/14).
