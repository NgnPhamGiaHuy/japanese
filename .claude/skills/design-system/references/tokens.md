...
Per-hue border/hover colors are wired into the `@theme` block as literal-value tokens, **not** `color-mix()` derivations: `hiragana-strong #58a700` / `hiragana-hover #46a302` · `katakana-strong #1899d6` / `katakana-hover #149fdf` · `both-strong #b65ce8` · `survival-strong #cc7800` / `survival-hover #e68700` · `danger-strong #b82222` / `danger-hover #d92626` · `muted-strong #8f8f8f` / `muted-hover #9f9f9f` · `teal-strong #00a8b5` / `teal-hover #00b8ca` · `pink-strong #e056a4` / `pink-hover #ff88cc`.

`color-mix()` was tried first and rejected: these shades are hand-picked per hue, not a uniform darken. Checking the channel ratios (e.g. `hiragana` `#58cc02` → `hiragana-strong` `#58a700` keeps R constant, drops G by ~19%, drops B to 0) and the HSL hue (94.5° → 88.4°, a real ~6° shift) confirms no single `color-mix()` recipe reproduces all 7 hues' shipped values — mixing toward black/gray preserves the R:G:B ratio (or hue, in HSL/OKLCH), and these don't preserve either. Literal tokens keep pixel-fidelity with the already-shipped design while still giving each hue one source of truth — `Button.tsx`'s `THEMES` map and ~15 other files that used to hardcode these hex values now reference the tokens directly.
